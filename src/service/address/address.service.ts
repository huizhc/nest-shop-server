...
export class AddressService {
    ...
    async add(json:Addressinterface){
        try {
            var access=new this.addressModel(json);
    ...
    async update(json1:Addressinterface,json2:Addressinterface){
        try {
            var result=await this.addressModel.updateOne(json1,json2);          
            return result;
        } catch (error) {
            return null;
        }
    ...
    async updateMany(json1:Addressinterface,json2:Addressinterface){
        try {
            var result=await this.addressModel.updateMany(json1,json2);          
            return result;
        } catch (error) {
            return null;
        }
    ...
    async delete(json:Addressinterface){
        try {
            var result=await this.addressModel.deleteOne(json);          
            return result;
        } catch (error) {
            return null;
        }
